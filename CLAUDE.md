...
## 성장 (학습 누적)
우리팀의 뇌는 이 레포의 `team-memory/`다 (직접 설치 시 함께 깔림).
- 팀 모드 시작 시 team-lead가 `./team-memory/`를 훑어 관련 과거 패턴을 끌어온다(원점에서 시작하지 않음).
- 큰 작업/프로젝트 종료 시 `/retro`(또는 "회고하자")로 회고를 쓰고, 본체로 옮길 학습은 `jiwon-harness`에 PR한다.
- 테스트/빌드를 돌렸다면 team-lead는 `.jiwon-team/last-test-output.txt`(PostToolUse 훅이 적재)를 증거로 읽고 완료를 단언한다.
